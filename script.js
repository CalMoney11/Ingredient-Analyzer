...
async function getRecipes() {
    ...
    try {
        // Fetch from the configured backend URL
        ...
        if (result.success && result.ingredients) {
            ...
            outputDiv.innerHTML = `
                <div class="space-y-4">
                    ...
                    <p class="text-blue-600 font-medium">Generating recipes...</p>
                </div>
            `;
            ...
            if (recipesResult.success && recipesResult.recipes) {
                ...
                if (recipes.length === 0) {
                    ...
                    // 1. UPDATE: Display recipes using the new Gemini structure (title, ingredients list, instructions list)
                    const recipesHTML = recipes.map((recipe, idx) => {
                        const recipeIngredients = recipe.ingredients || [];
                        const instructions = recipe.instructions || [];
                        
                        // New: List of ingredients with quantities/units
                        const ingredientsListHTML = recipeIngredients.map(ing => 
                            `<li class="text-sm text-gray-600">• ${ing}</li>`
                        ).join('');
                        
                        // New: List of instructions
                        const instructionsListHTML = instructions.map((step, stepIdx) =>
                            `<li class="text-sm text-gray-700">${stepIdx + 1}. ${step}</li>`
                        ).join('');
                        
                        return `
                            <div class="border border-gray-200 rounded-lg p-4 bg-white shadow-sm">
                                <h4 class="text-lg font-semibold text-blue-700 mb-3">${idx + 1}. ${recipe.title}</h4>
                                
                                <h5 class="text-md font-medium text-gray-800 mb-1">Ingredients:</h5>
                                <ul class="space-y-1 mb-3 list-none">
                                    ${ingredientsListHTML}
                                </ul>
                                
                                <h5 class="text-md font-medium text-gray-800 mb-1">Instructions:</h5>
                    ...
                    // 2. UPDATE: Change the message and remove total_found display
                    outputDiv.innerHTML = `
                        <div class="space-y-4">
        ...
        outputDiv.innerHTML = `
            <div class="text-red-600 p-4 bg-red-50 rounded-lg">
                ...
                <ul class="text-sm list-disc ml-4 mt-1">
                    <li>Make sure your Cloud Run backend is deployed</li>
